...
def make_mesh_full(
    ...
    Args:
        ...
        length_v_e (float): Half-length of the electron velocity domain.
        length_v_i (float): Half-length of the ion velocity domain.
    ...
    Returns:
        MeshFull: A full mesh object containing grid points and
            cell edges for electrons and ions.
    """
    ...
    vs_e_edges = jnp.linspace(
        -length_v_e - dv_e / 2, length_v_e + dv_e / 2, nv + 1, endpoint=True
    )

    vs_i = jnp.linspace(-length_v_i, length_v_i, nv, endpoint=True)
    dv_i = float(vs_i[1] - vs_i[0])
    vs_i_edges = jnp.linspace(
        -length_v_i - dv_i / 2, length_v_i + dv_i / 2, nv + 1, endpoint=True
    )
    ...
    return MeshFull(
        ...
        dz=dz,
        vs_e=vs_e,
        vs_e_edges=vs_e_edges,
        dv_e=dv_e,
        vs_i=vs_i,
        vs_i_edges=vs_i_edges,
        dv_i=dv_i,
        mus=mus,
        dmu=dmu,
        Z=Z,
        ...
        MU=MU,
        period_z=length_z,
        period_v_e=length_v_e,
        period_v_i=length_v_i,
        period_mu=length_mu,
        nz=nz,
        nv=nv,
        nmu=nmu,
...
class VlasovPoissonSolverFull(eqx.Module):
    ...
    def build_semilag_z_e(self) -> Callable[[Array], Array]:
        ...
        def interp_jax_z_e(f: Array) -> Array:
            def advect_z_1d(v_val, f_1d):
                ...
                # PCHIP on the primitive function guarantees strictly
                # monotonic mass (no wiggles)
    ...
    def build_semilag_v_e(self) -> Callable[[Array, Array, Array], Array]:
        ...
        def interp_jax_v_e(f: Array, E: Array, partial_B: Array) -> Array:
            def advect_v_1d(e_val, pb_val, mu_val, f_1d):
                ...
                safe_f = jnp.maximum(f_1d, 1e-8)
                mass = (
                    safe_f * self.mesh.dv_e
                )  # <--- Prevents flat CDFs and NaN gradients
                F_edges = jnp.concatenate([jnp.zeros(1), jnp.cumsum(mass)])

                interpolator = interpax.PchipInterpolator(
                    self.mesh.vs_e_edges, F_edges, check=False
                )
    ...
    def build_semilag_v_i(self) -> Callable[[Array, Array, Array], Array]:
        ...
        def interp_jax_v_i(f: Array, E: Array, partial_B: Array) -> Array:
            def advect_v_1d(e_val, pb_val, mu_val, f_1d):
                ...
                safe_f = jnp.maximum(f_1d, 1e-8)
                mass = (
                    safe_f * self.mesh.dv_i
                )  # <--- Prevents flat CDFs and NaN gradients
                F_edges = jnp.concatenate([jnp.zeros(1), jnp.cumsum(mass)])

                interpolator = interpax.PchipInterpolator(
                    self.mesh.vs_i_edges, F_edges, check=False
                )
                F_src = interpolator(v_edges_src)
    ...
    def compute_rho_e(self, B: Array, f: Array) -> Array:
        """Computes the full charge density for electrons.

        Args:
            B (Array): The magnetic field.
            f (Array): The electron distribution function.

        Returns:
            Array: The computed electron charge density.
        """
        return (
            2
            * jnp.pi
            * B
            * jnp.trapezoid(
                jnp.trapezoid(f, self.mesh.vs_e, axis=1), self.mesh.mus, axis=1
            )
        )

    def compute_rho_i(self, B: Array, f: Array) -> Array:
        """Computes the full charge density for ions.

        Args:
            B (Array): The magnetic field.
            f (Array): The ion distribution function.

        Returns:
            Array: The computed ion charge density.
        """
        return (
            2
            * jnp.pi
            * B
            * jnp.trapezoid(
                jnp.trapezoid(f, self.mesh.vs_i, axis=1), self.mesh.mus, axis=1
            )
        )

    def compute_rho_1d_e(self, f: Array) -> Array:
        """Computes the 1D charge density along z for electrons.

        Args:
            f (Array): The electron distribution function.

        Returns:
            Array: The computed 1D electron charge density.
        """
    ...
    def compute_rho_1d_i(self, f: Array) -> Array:
        """Computes the 1D charge density along z for ions.

        Args:
            f (Array): The ion distribution function.

        Returns:
            Array: The computed 1D ion charge density.
        """
        return jnp.trapezoid(
            jnp.trapezoid(f, self.mesh.vs_i, axis=1), self.mesh.mus, axis=1
        )
    ...
    def compute_phi_from_rho(
        self, A: Array, B: Array, rho_e: Array, rho_i: Array, rho_net_0: Array
    ) -> Array:
        """Computes the electric potential phi from the charge densities.

        Args:
            A (Array): The matrix A for solving Poisson's equation.
            B (Array): The magnetic field.
            rho_e (Array): The electron charge density.
            rho_i (Array): The ion charge density.
            rho_net_0 (Array): The initial net charge density.
    ...
    def compute_E(
        ...
        """Computes the electric field directly from the distribution
        functions.

        Args:
            f_e (Array): The electron distribution function.
            f_i (Array): The ion distribution function.
    ...
    def run_forward_jax_scan(
        ...
        """Runs the forward simulation using JAX's lax.scan for multi-species.

        Args:
            f_e_iv (jax.Array): The initial electron distribution function.
            f_i_iv (jax.Array): The initial ion distribution function.
            B (jax.Array): The magnetic field.
            partial_B (jax.Array): The spatial derivative of the
                magnetic field.
            g (jax.Array): The gradient of the magnetic field
                over the magnetic field.
        ...
        Returns:
            tuple: A tuple containing final distributions, E array, ee array,
            rho_e array, and rho_i array.
        """
        ...
        tspan_chunks = tspan[: num_chunks * chunk_size].reshape(
        ...
        if remainder > 0:
            ...
            E_total_array = jnp.concatenate(
                [E_total_array, rem_arrays[0]], axis=0
            )
        ...
        rho_e_array = jnp.concatenate(
            [rho_e_0_1d[None, :], rho_e_array], axis=0
        )
        rho_i_array = jnp.concatenate(
            [rho_i_0_1d[None, :], rho_i_array], axis=0
        )
    ...
    def run_forward_jax_scan_no_E(
        ...
        Args:
            f_e_iv (Array): The initial electron distribution function.
            f_i_iv (Array): The initial ion distribution function.
            B (Array): The magnetic field.
        ...
        Returns:
            tuple: A tuple containing final distributions and rho arrays.
        """
        num_steps = int(t_final / self.dt)
        f_e = f_e_iv.copy()
        f_i = f_i_iv.copy()
    ...
    def run_forward_jax_scan_efficient(
        ...
        Returns:
            tuple: A tuple containing final electron and ion distributions.
        """
        num_steps = int(t_final / self.dt)
        f_e = f_e_iv.copy()
        f_i = f_i_iv.copy()
        ...
        carry_final, _ = jax.lax.scan(chunk_step_jax, (f_e, f_i), tspan_chunks)
    ...
    def run_forward_hybrid(
        ...
        """Runs the forward simulation using a hybrid CPU-GPU approach for
        large times.
        ...
        Returns:
            tuple: A tuple containing final distributions and
                trajectory arrays.
        """
        num_steps = int(t_final / self.dt)
        f_e = f_e_iv
        ...
        # Helper function to pull a chunk's results off the GPU
        # and into system RAM
        def append_to_cpu(chunk_arrays_gpu):
            # jax.device_get is the critical step: it pulls data
            # from GPU VRAM -> CPU RAM
        ...
        # 5. Concatenate everything together on the CPU side using
        # standard NumPy
        ...
        # Now concatenate the lists of CPU arrays into continuous
        ...
        rho_e_array = np.concatenate(
            [rho_e_0_1d_cpu[None, ...]] + cpu_rho_e, axis=0
        )
        rho_i_array = np.concatenate(
            [rho_i_0_1d_cpu[None, ...]] + cpu_rho_i, axis=0
        )
