...
def plot_magnetic_fields(
    B_eval: Array, dB_eval: Array, g_eval: Array, zs: Array
) -> None:
...
def plot_initial_distribution_single(f_iv: Array, mesh: Mesh) -> None:
    ...
    im = axs[0].imshow(
        f_iv[:, :, 0].T,
        origin="lower",
        extent=[mesh.zs[0], mesh.zs[-1], mesh.vs[0], mesh.vs[-1]],
        aspect="auto",
        cmap="plasma",
    )
    axs[0].set_xlabel("$z$", fontsize=20)
...
def plot_final_distribution_single(
    f_array_1: Array, f_array_2: Array, mesh: Mesh
) -> None:
    ...
    axs[0, 1].set_title(
        "$f_e(T,z,v,{:.3f})$".format(mesh.mus[int(mesh.nmu / 16)])
    )
    ...
    # axs[1,1].set_ylabel("$v$")
    # axs[1,1].set_title(
    #     "$f_e(T,z,v,{:.3f})$".format(mesh.mus[int(mesh.nmu/2)])
    # )
...
def plot_energies_single(
    t_values: jnp.ndarray,
    ee_array: jnp.ndarray,
    E_array: jnp.ndarray,
    dB_eval: jnp.ndarray,
    mesh: Mesh,
) -> None:
    ...
    axs[1].plot(
        mesh.zs, E_array[0, :], label="$E({:.1f},z)$".format(t_values[0])
    )
    axs[1].plot(
        mesh.zs, E_array[5, :], label="$E({:.1f},z)$".format(t_values[5])
    )
    axs[1].plot(
        mesh.zs, E_array[10, :], label="$E({:.1f},z)$".format(t_values[10])
    )
    axs[1].plot(
        mesh.zs, E_array[50, :], label="$E({:.1f},z)$".format(t_values[50])
    )
    axs[1].plot(
        mesh.zs, E_array[-1, :], label="$E({:.1f},z)$".format(t_values[-1])
    )
...
def plot_E_effect_rhos_single(
    rho_array: Array,
    rho_array_no_E: Array,
    B_eval: Array,
    t_values: Array,
    mesh: Mesh,
) -> None:
    ...
    line = matplotlib.lines.Line2D(
        [0], [0], color="k", linestyle="--", label="No $E$"
    )

    axs[0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_array[0, :],
        color=cmap(0),
        label="$\\rho({:.2f},z)$".format(t_values[0]),
    )
    axs[0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_array_no_E[0, :],
        color=cmap(0),
        linestyle="--",
    )
    axs[0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_array[10, :],
        color=cmap(1),
        label="$\\rho({:.2f},z)$".format(t_values[10]),
    )
    axs[0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_array_no_E[10, :],
        color=cmap(1),
        linestyle="--",
    )
    axs[0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_array[100, :],
        color=cmap(2),
        label="$\\rho({:.2f},z)$".format(t_values[100]),
    )
    axs[0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_array_no_E[100, :],
        color=cmap(2),
        linestyle="--",
    )
    axs[0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_array[-1, :],
        color=cmap(3),
        label="$\\rho({:.2f},z)$".format(t_values[-1]),
    )
    axs[0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_array_no_E[-1, :],
        color=cmap(3),
        linestyle="--",
    )
...
def plot_rhos_single(
    rho_array: Array, B_eval: Array, t_values: Array, mesh: Mesh
) -> None:
    fig, axs = plt.subplots(1, 2, figsize=(22, 7))
...
def plot_E_effect_int_rho_single(
    rho_array: Array,
    rho_array_no_E: Array,
    t_values: Array,
    LZ: float,
...
def plot_int_rho_single(
    rho_array_initial: Array,
    rho_array_optimized: Array,
    t_values: Array,
    LZ: float,
    LV: float,
    prop_trapped: float,
    ...
    ax.plot(
        t_values,
        vcost_rho(rho_array_initial, mesh.zs),
        label="Initial $|\\mathbf{B}|$",
    )
    ax.plot(
        t_values,
        vcost_rho(rho_array_optimized, mesh.zs),
        label="Optimized $|\\mathbf{B}|$",
    )
    ax.set_title("$\\int \\rho_e^{1D}(t,z)\\mathrm{d}z$")
    ax.set_xlabel("$t$", fontsize=20)
    ax.hlines(
...
def plot_summray_opt_single(
    B_eval_init: Array,
    B_eval_opt: Array,
    t_values: Array,
    rho_array_init: Array,
    rho_array_opt: Array,
    E_array_init: Array,
    E_array_opt: Array,
    mesh: Mesh,
    ...
    line = matplotlib.lines.Line2D(
        [0], [0], color="k", linestyle="--", label="$\\theta_0$"
    )

    cmap = plt.get_cmap("tab10")

    axs[0, 0].plot(
        mesh.zs, B_eval_opt, color=cmap(0), label="$\\theta^{\\ast}$"
    )
    axs[0, 0].plot(
        mesh.zs,
        B_eval_init,
    ...
    axs[0, 1].plot(
        t_values,
        vcost_rho(rho_array_opt, mesh.zs),
        label="$\\theta^{\\ast}$",
        color=cmap(0),
    )
    axs[0, 1].plot(
        t_values,
        vcost_rho(rho_array_init, mesh.zs),
        label="$\\theta_0$",
        color=cmap(0),
        linestyle="--",
    )
    axs[0, 1].vlines(
        x=LZ / LV,
        ymin=jnp.min(vcost_rho(rho_array_init, mesh.zs)),
        ymax=1.0,
        colors="r",
        linestyles="--",
        label="Minimum time before leak",
    ...
    axs[1, 0].plot(
        mesh.zs, E_array_opt[10, :], label="$E({:.1f},z)$".format(t_values[10])
    )
    axs[1, 0].plot(mesh.zs, E_array_init[10, :], color=cmap(0), linestyle="--")
    axs[1, 0].plot(
        mesh.zs, E_array_opt[40, :], label="$E({:.1f},z)$".format(t_values[40])
    )
    axs[1, 0].plot(mesh.zs, E_array_init[40, :], color=cmap(1), linestyle="--")
    axs[1, 0].plot(
        mesh.zs, E_array_opt[70, :], label="$E({:.1f},z)$".format(t_values[70])
    )
    axs[1, 0].plot(mesh.zs, E_array_init[70, :], color=cmap(2), linestyle="--")
    axs[1, 0].plot(
        mesh.zs,
        E_array_opt[150, :],
        label="$E({:.1f},z)$".format(t_values[150]),
    )
    axs[1, 0].plot(
        mesh.zs, E_array_init[150, :], color=cmap(3), linestyle="--"
    )
    ...
    axs[1, 1].plot(
        ...
        rho_array_opt[0, :],
        color=cmap(0),
        label="$\\rho_e({:.2f},z)$".format(t_values[0]),
    )
    axs[1, 1].plot(
        mesh.zs, rho_array_init[0, :], color=cmap(0), linestyle="--"
    )
    axs[1, 1].plot(
        mesh.zs,
        rho_array_opt[50, :],
        color=cmap(1),
        label="$\\rho_e({:.2f},z)$".format(t_values[50]),
    )
    axs[1, 1].plot(
        mesh.zs, rho_array_init[50, :], color=cmap(1), linestyle="--"
    )
    axs[1, 1].plot(
        mesh.zs,
        rho_array_opt[100, :],
        color=cmap(2),
        label="$\\rho_e({:.2f},z)$".format(t_values[100]),
    )
    axs[1, 1].plot(
        mesh.zs, rho_array_init[100, :], color=cmap(2), linestyle="--"
    )
    axs[1, 1].plot(
        mesh.zs,
        rho_array_opt[-1, :],
        color=cmap(3),
        label="$\\rho_e({:.2f},z)$".format(t_values[-1]),
    )
    axs[1, 1].plot(
        mesh.zs, rho_array_init[-1, :], color=cmap(3), linestyle="--"
    )
    axs[1, 1].set_title(
        "$\\rho_e^{1D}(t,z;\\theta_0)$ and $\\rho_e^{1D}(t,z;\\theta^{\\ast})$"
    )
    axs[1, 1].set_xlabel("$z$", fontsize=20)
...
def plot_initial_distribution_multi(
    f_iv_e: Array, f_iv_i: Array, mesh: MeshFull
) -> None:
    fig, axs = plt.subplots(2, 3, figsize=(28, 15))
    ...
    # axs[0,1].set_ylabel("$v$")
    axs[0, 1].set_title(
        "$f_e(0,z,v,{:.3f})$".format(mesh.mus[int(mesh.nmu / 2)])
    )
    fig.colorbar(im, ax=axs[0, 1], fraction=0.046, pad=0.04)
    ...
    axs[1, 1].set_title(
        "$f_i(0,z,v,{:.3f})$".format(mesh.mus[int(mesh.nmu / 2)])
    )
    fig.colorbar(im, ax=axs[1, 1], fraction=0.046, pad=0.04)
...
def plot_final_distribution_multi(
    f_e_array: Array,
    f_i_array: Array,
    norm_il,
    norm_ic,
    ...
    im_ec = axs[0, 1].imshow(
        f_e_array[:, :, int(mesh.nmu / 16)].T,
        origin="lower",
        extent=[mesh.zs[0], mesh.zs[-1], mesh.vs_e[0], mesh.vs_e[-1]],
        aspect="auto",
        cmap="plasma",
    ...
    im_ic = axs[1, 1].imshow(
        f_i_array[:, :, int(mesh.nmu / 16)].T,
        origin="lower",
        extent=[mesh.zs[0], mesh.zs[-1], mesh.vs_i[0], mesh.vs_i[-1]],
        aspect="auto",
        cmap="plasma",
    ...
    # axs[1,1].set_ylabel("$v$")
    axs[1, 1].set_title(
        "$f_i(T,z,v,{:.3f})$".format(mesh.mus[int(mesh.nmu / 16)])
    )

    im_ir = axs[1, 2].imshow(
        f_i_array[:, :, -1].T,
        origin="lower",
        extent=[mesh.zs[0], mesh.zs[-1], mesh.vs_i[0], mesh.vs_i[-1]],
        aspect="auto",
...
def plot_energies_multi(
    ...
    axs[1].plot(
        mesh.zs, E_total_array[0, :], label="$E({:.1f},z)$".format(t_values[0])
    )
    axs[1].plot(
        mesh.zs,
        E_total_array[40, :],
        label="$E({:.1f},z)$".format(t_values[40]),
    )
    axs[1].plot(
        mesh.zs,
        E_total_array[80, :],
        label="$E({:.1f},z)$".format(t_values[80]),
    )
    axs[1].plot(
        mesh.zs,
        E_total_array[150, :],
    ...
    axs[1].plot(
        mesh.zs,
        E_total_array[500, :],
        label="$E({:.1f},z)$".format(t_values[500]),
    )
    axs[1].plot(
        mesh.zs,
        E_total_array[-1, :],
...
def plot_E_effect_rhos_multi(
    ...
    axs[0, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_e_array[0, :],
        color=cmap(0),
        label="$\\rho({:.2f},z)$".format(t_values[0]),
    )
    axs[0, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_e_array_no_E[0, :],
        color=cmap(0),
        linestyle="--",
    )
    axs[0, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_e_array[10, :],
        color=cmap(1),
        label="$\\rho({:.2f},z)$".format(t_values[10]),
    )
    axs[0, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_e_array_no_E[10, :],
        color=cmap(1),
        linestyle="--",
    )
    axs[0, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_e_array[100, :],
        color=cmap(2),
        label="$\\rho({:.2f},z)$".format(t_values[100]),
    )
    axs[0, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_e_array_no_E[100, :],
        color=cmap(2),
        linestyle="--",
    )
    axs[0, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_e_array[250, :],
        color=cmap(3),
        label="$\\rho({:.2f},z)$".format(t_values[250]),
    )
    axs[0, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_e_array_no_E[250, :],
        color=cmap(3),
        linestyle="--",
    )
    axs[0, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_e_array[-1, :],
        color=cmap(4),
        label="$\\rho({:.2f},z)$".format(t_values[-1]),
    )
    axs[0, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_e_array_no_E[-1, :],
        color=cmap(4),
        linestyle="--",
    )
    ...
    axs[0, 1].plot(
        mesh.zs, rho_e_array_no_E[0, :], color=cmap(0), linestyle="--"
    )
    axs[0, 1].plot(
        mesh.zs,
        rho_e_array[10, :],
    ...
    axs[0, 1].plot(
        mesh.zs, rho_e_array_no_E[10, :], color=cmap(1), linestyle="--"
    )
    axs[0, 1].plot(
        mesh.zs,
        rho_e_array[100, :],
    ...
    axs[0, 1].plot(
        mesh.zs, rho_e_array_no_E[100, :], color=cmap(2), linestyle="--"
    )
    axs[0, 1].plot(
        mesh.zs,
        rho_e_array[250, :],
    ...
    axs[0, 1].plot(
        mesh.zs, rho_e_array_no_E[250, :], color=cmap(3), linestyle="--"
    )
    axs[0, 1].plot(
        mesh.zs,
        rho_e_array[-1, :],
    ...
    axs[0, 1].plot(
        mesh.zs, rho_e_array_no_E[-1, :], color=cmap(4), linestyle="--"
    )
    ...
    axs[1, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_i_array[0, :],
        color=cmap(0),
        label="$\\rho({:.2f},z)$".format(t_values[0]),
    )
    axs[1, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_i_array_no_E[0, :],
        color=cmap(0),
        linestyle="--",
    )
    axs[1, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_i_array[10, :],
        color=cmap(1),
        label="$\\rho({:.2f},z)$".format(t_values[10]),
    )
    axs[1, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_i_array_no_E[10, :],
        color=cmap(1),
        linestyle="--",
    )
    axs[1, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_i_array[100, :],
        color=cmap(2),
        label="$\\rho({:.2f},z)$".format(t_values[100]),
    )
    axs[1, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_i_array_no_E[100, :],
        color=cmap(2),
        linestyle="--",
    )
    axs[1, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_i_array[250, :],
        color=cmap(3),
        label="$\\rho({:.2f},z)$".format(t_values[250]),
    )
    axs[1, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_i_array_no_E[250, :],
        color=cmap(3),
        linestyle="--",
    )
    axs[1, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_i_array[-1, :],
        color=cmap(4),
        label="$\\rho({:.2f},z)$".format(t_values[-1]),
    )
    axs[1, 0].plot(
        mesh.zs,
        2 * jnp.pi * B_eval * rho_i_array_no_E[-1, :],
        color=cmap(4),
        linestyle="--",
    )
    ...
    axs[1, 1].plot(
        mesh.zs, rho_i_array_no_E[0, :], color=cmap(0), linestyle="--"
    )
    axs[1, 1].plot(
        mesh.zs,
        rho_i_array[10, :],
    ...
    axs[1, 1].plot(
        mesh.zs, rho_i_array_no_E[10, :], color=cmap(1), linestyle="--"
    )
    axs[1, 1].plot(
        mesh.zs,
        rho_i_array[100, :],
    ...
    axs[1, 1].plot(
        mesh.zs, rho_i_array_no_E[100, :], color=cmap(2), linestyle="--"
    )
    axs[1, 1].plot(
        mesh.zs,
        rho_i_array[250, :],
    ...
    axs[1, 1].plot(
        mesh.zs, rho_i_array_no_E[250, :], color=cmap(3), linestyle="--"
    )
    axs[1, 1].plot(
        mesh.zs,
        rho_i_array[-1, :],
    ...
    axs[1, 1].plot(
        mesh.zs, rho_i_array_no_E[-1, :], color=cmap(4), linestyle="--"
    )
...
def plot_E_effect_int_rho_multi(
    rho_e_array: Array,
    rho_e_array_no_E: Array,
    rho_i_array: Array,
    rho_i_array_no_E: Array,
...
def plot_int_rho_multi(
    rho_e_array_init: Array,
    ...
    axs[0].plot(
        ts,
        vcost_rho(rho_e_array_init, mesh.zs),
        label="Initial $|\\mathbf{B}|$",
    )
    axs[0].plot(
        ts,
        vcost_rho(rho_e_array_opt, mesh.zs),
        label="Optimized $|\\mathbf{B}|$",
    )
    axs[0].set_title("$\\int \\rho_e^{1D}(t,z)\\mathrm{d}z$")
    axs[0].set_xlabel("$t$", fontsize=20)
    ...
    axs[1].plot(
        ts,
        vcost_rho(rho_i_array_init, mesh.zs),
        label="Initial $|\\mathbf{B}|$",
    )
    axs[1].plot(
        ts,
        vcost_rho(rho_i_array_opt, mesh.zs),
        label="Optimized $|\\mathbf{B}|$",
    )
    axs[1].set_title("$\\int \\rho_i^{1D}(t,z)\\mathrm{d}z$")
    axs[1].set_xlabel("$t$", fontsize=20)
...
def plot_summray_opt_multi(
    ...
    axs[0, 1].vlines(
        x=LZ / LV,
        ymin=jnp.min(vcost_rho(rho_i_array_init, mesh.zs)),
        ymax=1.0,
        colors="r",
        linestyles="--",
        label="Minimum time before leak (e)",
    )
    axs[0, 1].vlines(
        x=(LZ / LV) * jnp.sqrt(m_i),
        ymin=jnp.min(vcost_rho(rho_i_array_init, mesh.zs)),
        ...
        colors="b",
        linestyles="--",
        label="Minimum time before leak (i)",
    )
    axs[0, 1].set_xlabel("$t$", fontsize=20)
    ...
    axs[1, 0].plot(
        mesh.zs,
        E_array_opt[100, :],
        label="$E({:.1f},z)$".format(t_values[100]),
    )
    axs[1, 0].plot(mesh.zs, E_array_init[400, :], linestyle="--")
    axs[1, 0].plot(
        mesh.zs,
        E_array_opt[400, :],
        label="$E({:.1f},z)$".format(t_values[400]),
    )
    axs[1, 0].plot(mesh.zs, E_array_init[650, :], linestyle="--")
    axs[1, 0].plot(
        mesh.zs,
        E_array_opt[650, :],
        label="$E({:.1f},z)$".format(t_values[650]),
    )
    ...
    axs[1, 1].plot(
        t_values[1:], ee_array_opt, color=cmap(0), label="$\\theta^{\\ast}$"
    )
    ...
    axs[2, 0].plot(
        mesh.zs,
        rho_e_array_opt[0, :],
        label="$\\rho_e({:.2f},z;\\theta^\\ast)$".format(t_values[0]),
    )
    axs[2, 0].plot(
        mesh.zs,
        rho_e_array_opt[50, :],
        label="$\\rho_e({:.2f},z;\\theta^\\ast)$".format(t_values[50]),
        color=cmap(1),
    )
    axs[2, 0].plot(
        mesh.zs,
        rho_e_array_opt[100, :],
        label="$\\rho_e({:.2f},z;\\theta^\\ast)$".format(t_values[100]),
        color=cmap(2),
    )
    axs[2, 0].plot(
        mesh.zs,
        rho_e_array_opt[-1, :],
        label="$\\rho_e({:.2f},z;\\theta^\\ast)$".format(t_values[-1]),
        color=cmap(3),
    )
    axs[2, 0].plot(
        mesh.zs,
        rho_e_array_init[0, :],
        label="$\\rho_e({:.2f},z;\\theta_0)$".format(t_values[0]),
        linestyle="--",
    )
    axs[2, 0].plot(
        mesh.zs,
        rho_e_array_init[50, :],
        label="$\\rho_e({:.2f},z;\\theta_0)$".format(t_values[50]),
        color=cmap(1),
        linestyle="--",
    )
    axs[2, 0].plot(
        mesh.zs,
        rho_e_array_init[100, :],
        label="$\\rho_e({:.2f},z;\\theta_0)$".format(t_values[100]),
        color=cmap(2),
        linestyle="--",
    )
    axs[2, 0].plot(
        mesh.zs,
        rho_e_array_init[-1, :],
        label="$\\rho_e({:.2f},z;\\theta_0)$".format(t_values[-1]),
        color=cmap(3),
        linestyle="--",
    )
    ...
    axs[2, 1].plot(
        mesh.zs,
        rho_i_array_opt[0, :],
        label="$\\rho_i({:.2f},z;\\theta^\\ast)$".format(t_values[0]),
    )
    axs[2, 1].plot(
        mesh.zs,
        rho_i_array_opt[50, :],
        label="$\\rho_i({:.2f},z;\\theta^\\ast)$".format(t_values[50]),
        color=cmap(1),
    )
    axs[2, 1].plot(
        mesh.zs,
        rho_i_array_opt[100, :],
        label="$\\rho_i({:.2f},z;\\theta^\\ast)$".format(t_values[100]),
        color=cmap(2),
    )
    axs[2, 1].plot(
        mesh.zs,
        rho_i_array_opt[-1, :],
        label="$\\rho_i({:.2f},z;\\theta^\\ast)$".format(t_values[-1]),
        color=cmap(3),
    )
    axs[2, 1].plot(
        mesh.zs,
        rho_i_array_init[0, :],
        label="$\\rho_i({:.2f},z;\\theta_0)$".format(t_values[0]),
        linestyle="--",
    )
    axs[2, 1].plot(
        mesh.zs,
        rho_i_array_init[50, :],
        label="$\\rho_i({:.2f},z;\\theta_0)$".format(t_values[50]),
        color=cmap(1),
        linestyle="--",
    )
    axs[2, 1].plot(
        mesh.zs,
        rho_i_array_init[100, :],
        label="$\\rho_i({:.2f},z;\\theta_0)$".format(t_values[100]),
        color=cmap(2),
        linestyle="--",
    )
    axs[2, 1].plot(
        mesh.zs,
        rho_i_array_init[-1, :],
        label="$\\rho_i({:.2f},z;\\theta_0)$".format(t_values[-1]),
        color=cmap(3),
        linestyle="--",
    )
